...
const hash = await walletClient.sendTransaction({ account, chain, to: request.recipient, value });
const receipt = await publicClient.waitForTransactionReceipt({ hash });
if (receipt.status !== "success") throw new Error(`native transfer reverted: ${hash}`);

console.log(JSON.stringify({ transactionHash: hash, status: receipt.status }));

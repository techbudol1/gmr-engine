...
This is the privacy model for the current BudolPH private winning-claim circuit.
...
The frontend creates the private claim note and keeps the secret locally. BudolPH API receives only the public commitment leaf at trade time and the public nullifier at claim time.

GMR Engine still includes a helper script for development, testing, and root/circuit-input tooling, but the user-facing BudolPH flow no longer sends full note values to the backend.

## What Is Not Finished Yet

This implementation adds the circuit source, development proving artifacts, browser proof generation, BudolPH proof submission, ZKVerify submission through GMR Engine, and claim-time payout checks. BudolPH also has a separate shielded payout pool path for fixed-denomination withdrawals.
...
## Practical Meaning For BudolPH

With this circuit, BudolPH can evolve from public bet records into private claim records:
...
5. BudolPH pays the winner after confirming the proof and that the nullifier was not used before.
